...
!(async () => {
    ...
    if(process.env.COLLECT_SHARE_CODE_JS_FUNC)
    {
        functionArr=[process.env.COLLECT_SHARE_CODE_JS_FUNC]
    } else {
        functionArr = [
            'getJDFruit',
            'getJdPet',
    ...
    for(let j = 0; j < functionArr.length; j++) {

        console.log(`======开始`+functionArr[j]+`互助码======`)
        ...
        for (let i = 0; i < cookiesArr.length; i++) {
            if (cookiesArr[i]) {
                ...
                await getShareCode(functionArr[j])

            }
        }

        console.log(`======结束`+functionArr[j]+`互助码======`)
                        ...
                        if (safeGet(data)) {
                            ...
                            if (data.data.bizCode === 0) {
                                ...
                                $.taskVos.map((item) => {
                                    if (item.taskType === 14) {
                                        console.log(
                                            `【京东账号${$.index}（${$.UserName}）的东东工厂好友互助码】${item.assistTaskDetailVo.taskToken}`
                                        );
                                    }
                                });
                            }
                        ...
                        if (safeGet(data)) {
                            ...
                            if (data["ret"] === 0) {
                                ...
                                if (data.factoryList && data.productionList) {
                                    ...
                                    console.log(`【京东账号${$.index}（${$.UserName}）京喜工厂好友互助码】${data.user.encryptPin}`);
                                }
                            } else {
                        ...
                        if (safeGet(data)) {
                            ...
                            if (data["ret"] === 0) {
                                if (data.active) {
                                    ...
                                    console.log(`【京东账号${$.index}（${$.UserName}）的京喜农场好友互助码】` + JSON.stringify(shareCodeJson));
                                } else {
            ...
            try {
                if (err) {
                    ...
                    if (
                        ...
                        console.log(
                            `【京东账号${$.index}（${$.UserName}）的东东萌宠好友互助码】${$.petInfo.shareCode}`
                        );
...
async function getPlantBean() {
    ...
    async function jdPlantBean() {
        ...
        if ($.plantBeanIndexResult.code === "0") {
            ...
            console.log(`【京东账号${$.index}（${$.UserName}）的京东种豆得豆好友互助码】${$.myPlantUuid}`);
...
async function getJDFruit() {
    ...
    async function jdFruit() {
        ...
        if ($.farmInfo.farmUserPro) {
            console.log(
                `【京东账号${$.index}（${$.UserName}）的东东农场好友互助码】${$.farmInfo.farmUserPro.shareCode}`
            );
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.success && data.data && data.data.userInviteCode) {
                            console.log(`【京东账号${$.index}（${$.UserName}）的crazyJoy任务好友互助码】${data.data.userInviteCode}`)
                        }
                    }
                }
            } catch (e) {
            ...
            $.post(url, async (err, resp, data) => {
                try {
                    ...
                    if (data.data.bizCode === 0) {
                        ...
                        console.log(`【京东账号${$.index}（${$.UserName}）的闪购盲盒好友互助码】${invites && invites[0]['assistTaskDetailVo']['taskToken']}`)
                    }
                } catch (e) {
                    $.logErr(e, resp);
                } finally {
    ...
    return new Promise(async (resolve) => {
        $.get(taskUrl(`user/QueryUserInfo`), (err, resp, data) => {
            try {
                ...
                console.log(`【京东账号${$.index}（${$.UserName}）的京喜财富岛好友互助码 ${strMyShareId}`)
            } catch (e) {
                $.logErr(e, resp);
            } finally {
                resolve();
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if(data.code===0 && data.data.result){
                            console.log(`【京东账号${$.index}（${$.UserName}）的签到领现金好友互助码】${data.data.result.inviteCode}`);
                        }
                    }
                }
            } catch (e) {
...
async function getShareCode(executeFunc) {
